# Feature parity: entra-emulator vs. real Microsoft Entra ID — v0.2.2

This map was written after v0.2.2 shipped — the release predates the parity map
itself, and a git tag is immutable — so it is a retrospective reading rather
than a document published at the time. It is drawn from the v0.2.2 tree:
its routes, the packages present, and the CI suites it ran.

`main` sat exactly on `v0.2.2` when the map was first written, so this snapshot
