# Feature parity: entra-emulator vs. real Microsoft Entra ID — v0.1.2

This map was written after v0.1.2 shipped — the release predates the parity map
itself, and a git tag is immutable — so it is a retrospective reading rather
than a document published at the time. It is drawn from the v0.1.2 tree:
its routes, the packages present, and the CI suites it ran.

Parity-identical to v0.1.1 — the single commit in between was a winget release
fix. The shape of v0.1.x: the **protocol core was already real** — the RS256 token
