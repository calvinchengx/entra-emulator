# Feature parity: entra-emulator vs. real Microsoft Entra ID — v0.1.1

This map was written after v0.1.1 shipped — the release predates the parity map
itself, and a git tag is immutable — so it is a retrospective reading rather
than a document published at the time. It is drawn from the v0.1.1 tree:
its routes, the packages present, and the CI suites it ran.

Parity-identical to v0.1.0 — the five commits in between were docs, a winget
manifest and runnable samples. The shape of v0.1.x: the **protocol core was already real** — the RS256 token
service, all six grants, WebAuthn, managed identity, `private_key_jwt`, the
Fabric token layer, Graph directory CRUD and the PDP port. What later releases
added was reach: SCIM (v0.2.0), the seven-engine PDP matrix and the .NET/Java
SDK suites (v0.2.0), then the stateful directory and instance discovery
(v0.2.2).
...
| Entra feature | Emulator | Type |
|---|---|---|
| SCIM **service provider** (inbound): `ServiceProviderConfig`, `ResourceTypes`, `Schemas`; Users + Groups CRUD, PatchOp | — `internal/scim` does not exist | 🔴 Not implemented |
| SCIM **provisioning client** (outbound): filter-probe → create / update / `active:false` deprovision, member-correlated groups, incremental watermark | — `internal/scim` does not exist | 🔴 Not implemented |
