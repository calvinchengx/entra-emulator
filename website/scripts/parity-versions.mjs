...
// Match the parity map exactly — `docs/parity.md`, never `*-parity.md`. This
// repo also ships `19-golden-reference-parity.md` (mechanical conformance
// against the OIDC/Graph/SCIM goldens), which is a different document: a looser
// `-parity.md` pattern silently adopts it as the live map and snapshots it.
...
export function writeParityHistory(OUT, parity, helpers) {
  ...
  for (const pt of points) {
    ...
    const banner = pt.reconstructed
      ? ''
      : `:::note[Historical snapshot]\nThe feature-parity map as of release **${pt.label}**. The current map is on the [Parity page](/entra-emulator/${liveSlug}/).\n:::\n\n`;
  ...
  const clBody =
    `How the [feature-parity map](/entra-emulator/${liveSlug}/) changed across releases — ` +
  ...
  const rows = [
    `- **[${version}](/entra-emulator/${liveSlug}/)** — the live map on \`main\``,
    ...releasedPoints
      .slice()
      .reverse()
      .map(
        (p) =>
          `- [${p.label}](/entra-emulator/parity-history/${versionSlug(p.label)}/) — ` +
          (p.reconstructed ? 'written retrospectively (predates the map)' : 'snapshot at release'),
      ),
  ];
  const idxBody =
    `Versions of the [feature-parity map](/entra-emulator/${liveSlug}/), tracked by git release tags. ` +
    `See the [parity changelog](/entra-emulator/parity-history/changelog/) for what changed between them.\n\n` +
    rows.join('\n') +
    '\n\n' +
