// Boots the emulator over TLS and registers a same-origin redirect URI so
// Chromium can follow the implicit/hybrid 302 and we can read the fragment.
...
export default async function globalSetup() {
  const dir = mkdtempSync(join(tmpdir(), 'implicit-e2e-'));
  ...
  if (!spa) throw new Error('no seeded public client to drive implicit/hybrid');

  const redirectUri = `https://localhost:${EMU_PORT}/implicit-redirect`;
