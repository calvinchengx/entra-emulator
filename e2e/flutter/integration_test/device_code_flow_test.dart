...
/// Every request here talks to an emulator on the host over the loopback the
/// device is given, so a call that has not answered in seconds is not slow —
/// it is stuck. Without a bound the test blocks on the first stalled socket
/// and nothing stops it but the job's own 45-minute limit, having printed
/// nothing about which request died.
///
/// That is exactly what happened on 2026-08-19: the same commit that passed
/// the two previous nights, on the same runner image, built fine and then hung
/// for 37 minutes with no output. A per-step bound turns that into a named
/// failure in seconds.
const _stepTimeout = Duration(seconds: 20);

Future<T> _step<T>(String what, Future<T> Function() call) => call().timeout(
      _stepTimeout,
      onTimeout: () => throw TimeoutException(
          '$what did not answer within ${_stepTimeout.inSeconds}s — the '
          'emulator is probably unreachable from the device'),
    );
...
void main() {
  ...
  testWidgets('device-code flow issues verifiable tokens', (tester) async {
    ...
    final da = await _step('device authorization', () => http.post(
    ...
    final pending = await _step('pending poll', () => http.post(tokenUri, body: {
    ...
    final lookup = await _step('user-code lookup', () => http.post(verifyUri, body: {
          '__ee_step': 'lookup',
    ...
    final signinResp = await _step(
        'approval sign-in', () async => http.Response.fromStream(await signinReq.send()));
    ...
    final decide = await _step('approval decision', () => http.post(verifyUri,
    ...
    final tokens = await _step('token exchange', () => http.post(tokenUri, body: {
          'grant_type': 'urn:ietf:params:oauth:grant-type:device_code',
          'device_code': daBody['device_code'],
          'client_id': spaClientId,
        }));
    ...
    final me = await _step('graph /me', () => http.get(
    ...
    // A belt to the per-step braces: any stall outside a _step call still ends
    // as a failed test rather than as a killed job.
